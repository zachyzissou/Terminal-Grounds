...
{
	HandleFireTick();
	const float Rate = WeaponData ? FMath::Max(WeaponData->FireRate, 0.01f) : 0.1f;
	GetWorldTimerManager().SetTimer(FireTimer, this, &ATGWeapon::HandleFireTick, Rate, true);
}

void ATGWeapon::StopFire()
{
...
{
	FTGShotParams Params; Params.Timestamp = GetWorld()->GetTimeSeconds();
	if (HasAuthority())
	{
...
void ATGWeapon::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& Out) const
{
